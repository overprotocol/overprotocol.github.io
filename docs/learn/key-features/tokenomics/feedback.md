...
The primary function of the issuance reserve is to accumulate surplus issuance for future use. Specifically, it collects the difference between the planned issuance schedule and the fixed reward pool, serving as a resource for the feedback mechanism to augment rewards. When the actual rewards distributed are less than the scheduled issuance amount, the surplus is stored in the issuance reserve. In contrast, should the demand for rewards surpass the limits set by the issuance plan, additional funds are drawn from this reserve as needed. The management of the issuance reserve is handled at the protocol level, which protects it from risks such as account hacking and ensures that its use is strictly limited to the dynamic adjustment of yields.
